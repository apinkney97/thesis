...
\begin{figure}
    ...
    \subfloat[][A float 2-column float is encountered, and is inserted below the text]{\includegraphics[width=0.3\textwidth]{gfx/float3.png}}\hspace{0.04\textwidth}
...
The third option (backtracking and shunting) is likely to produce the most desirable output, although some computational overhead will be added. One approach is simply to check whether there is enough space immediately to the left (specifically a gap between other, already placed, floats) into which the current float can be placed, with the displaced lines being shunted forwards. This method will not produce layouts as optimal as methods that use full backtracking and check all possibilities, but will run in much quicker time. A combination of all three of the above options is likely to work best in practice.
...
In general, however, as a document layout engine, the system produces pleasing-looking layouts that scale to fit on many different screen sizes, and the reimplementation in \textsc{xhtml}, \textsc{css}, and JavaScript provides makes the system much more portable. One stand-out problem remains: that of ballooning filesizes. The next chapter suggests various methods to keep file sizes to a minimum.
