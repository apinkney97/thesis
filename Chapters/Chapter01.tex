\chapter{The rise of eBooks} \label{ch:intro} % ``Introduction'' is too boring!
...
\section{Devices}
\marginpar{Amazon\ed kindle apps for PC, iPhone, iPad, netbooks, *actual* kindles\ldots a plethora
...
\marginpar{Mention why tagged PDF/structure tree aren't particularly useful for reflow (ie helps to
unpick, but still need to re-typeset... no better off than html)}
...
EPUB and Mobipocket, both based on HTML, provide this necessary higher abstraction of documents.
...
EPUB\ed can embed fonts, can style with CSS pretty much arbitrarily, but still needs to be retypeset
on each viewing and thus relies on the crappy built in rendering engine of the device.}
